...
function signTypedData(signer: Wallet, data: EIP712TypedData) {
  const types = { ForwardRequest: forwardRequest } as Record<string, Array<TypedDataField>>

  return signer._signTypedData(data.domain, types, data.message)
}

const buildRequest = async (forwarder: Forwarder, input: Input):Promise<ForwardRequest> => {
...
export const signMetaTxRequest = async (signer: Wallet, forwarder: Forwarder, input: Input):Promise<{ request: ForwardRequest, signature: string }> => {
