...
describe('Check contract addresses', function() {
    ...
    it('Should return correct mainnet gateway token address by name, symbol and address', function() {
        ...
        let token: GatewayTokenItem = getGatewayTokenByName(gatewayTokenName, mainnetNetworkID);
    ...
    it('Should return correct ropsten gateway token address by name, symbol and address', function() {
        ...
        let token: GatewayTokenItem = getGatewayTokenByName(gatewayTokenName, ropstenNetworkID);
        assert.equal(token.address, actualAddress);
    ...
    it('Should return correct localhost gateway token address by name, symbol and address', function() {
        ...
        let token: GatewayTokenItem = getGatewayTokenByName(localhostTokenName, localhostNetworkID);
        assert.equal(token.address, actualAddress);
    ...
    it('Expect error on get addresses for non-existing network', function() {
        assert.throws(() => { getGatewayControllerByNetworkID(2) }, Error);
        assert.throws(() => { getFlagsStorageByNetworkID(2) }, Error);
        assert.throws(() => { getGatewayTokenByName(gatewayTokenName, 2) }, Error);
        assert.throws(() => { getGatewayTokenBySymbol(gatewayTokenSymbol, 2) }, Error);
        assert.throws(() => { getGatewayTokenByAddress("", 2) }, Error);
    });
});