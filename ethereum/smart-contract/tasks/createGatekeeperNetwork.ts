...
export const createGatekeeperNetwork = async (args: any, hre: HardhatRuntimeEnvironment) => {
  ...
  console.log(`created network ${gatekeeperNetwork} on Gateway Token at ${gatewayToken.address} using ${createTx.gasUsed.toNumber()} gas`);
  ...
  console.log(`added new gatekeeper ${gatekeeper} to network ${gatekeeperNetwork} using ${addGkTx.gasUsed.toNumber()} gas`);
};
