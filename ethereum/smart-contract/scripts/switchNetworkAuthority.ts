...
(async () => {
  ...
  let txReceipt = await contract.connect(oldNetworkAuthority).addNetworkAuthority(newNetworkAuthority.address, gatekeeperNetwork);
  let tx = await txReceipt.wait();
  console.log(`Added ${newNetworkAuthority.address} to network ${gatekeeperNetwork} using ${tx.gasUsed.toNumber()} gas`);

  txReceipt = await contract.connect(newNetworkAuthority).removeNetworkAuthority(oldNetworkAuthority.address, gatekeeperNetwork);
  tx = await txReceipt.wait();
  console.log(`Removed ${oldNetworkAuthority.address} from network ${gatekeeperNetwork} using ${tx.gasUsed.toNumber()} gas`);
})().catch((error) => {
    console.error(error);
    process.exit(1);
})
