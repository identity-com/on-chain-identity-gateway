...
describe('GatewayNetwork', () => {
    ...
    describe('Gatekeeper Network Update', async () => {
        ...
        it('can update network fees if primary authority', async () => {
            //given
            const updatedFees = {issueFee: 100, verificationFee: 100, expireFee: 100, refreshFee: 100, freezeFee: 100, revokeFee: 100};
            ...
            expect(updatedNetworkState.networkFee.revokeFee).to.eq(updatedFees.revokeFee);
        ...
        it('cannot update network fees if not primary authority', async () => {
            //given
            let updatedFees = {issueFee: 10001, verificationFee: 0, expireFee: 0, refreshFee: 0, revokeFee: 0, freezeFee: 0};
            const networkId = await gatekeeperNetworkContract.getNetworkId(defaultNetwork.name);
            const networkState = await gatekeeperNetworkContract.getNetwork(networkId);
            ...
            expect(networkState.networkFee.refreshFee).to.eq(0);
            expect(networkState.networkFee.expireFee).to.eq(0);
            expect(networkState.networkFee.revokeFee).to.eq(0);
            ...
            updatedFees =  {issueFee: 0, verificationFee: 10001, expireFee: 0, refreshFee: 0, revokeFee: 0, freezeFee: 0};
            ...
            updatedFees = {issueFee: 0, verificationFee: 0, expireFee: 10001, refreshFee: 0, revokeFee: 0, freezeFee: 0};
            ...
            updatedFees = {issueFee: 0, verificationFee: 0, expireFee: 0, refreshFee: 10001, revokeFee: 0, freezeFee: 0};
            ...
            updatedFees = {issueFee: 0, verificationFee: 0, expireFee: 0, refreshFee: 0, revokeFee: 0, freezeFee: 10001};
            ...
            updatedFees = {issueFee: 0, verificationFee: 0, expireFee: 0, refreshFee: 0, revokeFee: 10001, freezeFee: 0};
            await expect(gatekeeperNetworkContract.connect(primaryAuthority).updateFees(updatedFees, defaultNetwork.name, {gasLimit: 300000})).to.be.rejectedWith("Revoke fee must be below 100%");
        });
