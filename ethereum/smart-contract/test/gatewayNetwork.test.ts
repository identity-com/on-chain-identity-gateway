...
describe('GatewayNetwork', () => {
    ...
    describe('Gatekeeper Network Update', async () => {
        ...
        it('can update network fees if primary authority', async () => {
            //given
            const updatedFees = {issueFee: 100, verificationFee: 100, expireFee: 100, refreshFee: 100, freezeFee: 100};
        ...
        it('cannot update network fees if not primary authority', async () => {
            //given
            let updatedFees = {issueFee: 10001, verificationFee: 0, expireFee: 0, refreshFee: 0, freezeFee: 0};
            const networkId = await gatekeeperNetworkContract.getNetworkId(defaultNetwork.name);
            const networkState = await gatekeeperNetworkContract.getNetwork(networkId);
            ...
            updatedFees = {issueFee: 0, verificationFee: 0, expireFee: 10001, refreshFee: 0, freezeFee: 0};
            ...
            updatedFees = {issueFee: 0, verificationFee: 0, expireFee: 0, refreshFee: 10001, freezeFee: 0};
            ...
            updatedFees = {issueFee: 0, verificationFee: 0, expireFee: 0, refreshFee: 0, freezeFee: 10001};
        ...
        it('cannot update network fees if not enough time has passed since previous fee changes', async () => {
            //given
            let updatedFees = {issueFee: 100, verificationFee: 100, expireFee: 100, refreshFee: 100, freezeFee: 100};
            const networkId = await gatekeeperNetworkContract.getNetworkId(defaultNetwork.name);
            const networkState = await gatekeeperNetworkContract.getNetwork(networkId);
