...
describe("findGatewayToken integration tests", () => {
  ...
  before("Create a gatewayToken for the test account", async () => {
    ...
    await wait(
      chainUpdateDelay,
      `gatewayToken ${gatewayToken} created, waiting ${chainUpdateDelay}ms for the chain to update...`
    );
  });

  context("with a valid account", () => {
    it("should return the public key", async () => {
      const findGatewayTokenResponse = await findGatewayTokens(
        connection,
        owner,
        gatekeeperKey
      );
      expect(findGatewayTokenResponse[0].publicKey.toBase58()).to.eq(
        gatewayToken
      );
    });
  });
  ...
  context("with token accounts found", () => {
    context("with a frozen account", () => {
      ...
      beforeEach("revoke the test account gatewayToken", async () => {
        [revokedToken] = await findGatewayTokens(
          connection,
          owner,
          gatekeeperKey
        );
        ...
        await axios.delete(
          `${gatekeeperServerBaseUrl}/${revokedToken.publicKey.toBase58()}`
        );
        await wait(
          chainUpdateDelay,
          `token frozen, waiting ${chainUpdateDelay}ms for the chain to update...`
        );
      });
      it("should return an empty array", async () => {
        const findGatewayTokenResponse = await findGatewayTokens(
