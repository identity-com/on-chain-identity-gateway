...
use crate::{CreateNetworkData, NetworkKeyFlags, UpdateNetworkData};

#[derive(Debug)]
pub struct Network {}

impl Network {
    pub fn update(_data: UpdateNetworkData, _network: &mut Account<GatekeeperNetwork>) -> Result<()> {
        Ok(())
    }

    pub fn create(data: CreateNetworkData, network: &mut Account<GatekeeperNetwork>) -> Result<()> {
