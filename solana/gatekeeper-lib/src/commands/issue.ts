...
const store = (record: Record) => {
  const row = [record.timestamp, record.token, record.name, record.ipAddress, record.country, record.approved].join(',') + '\n'
...
`,
  ...
  async run() {
    ...
    const {gatekeeper, mintAccountPublicKey} = await getGatekeeper(connection)

    const service = new IssueService(connection, gatekeeper, mintAccountPublicKey)
